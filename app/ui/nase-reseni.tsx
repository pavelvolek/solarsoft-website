        ...
        <div className="space-y-8 md:grid md:grid-cols-2 md:gap-12 lg:grid-cols-4 md:space-y-0">
          {tiles.map((tile, index) => {
            return (
              ...
              >
                <div className={`relative grow-0 shrink-0 flex justify-center items-center w-14 h-14 rounded-xl shadow-md bg-gradient-to-br ${tile.color}`}>
                  <div
                    className={`absolute -top-2 -right-2 rounded-full w-7 h-7 text-base text-white font-semibold text-center border-2 border-white ${tile.color}`}
                  >
                    {index + 1}
                  </div>
